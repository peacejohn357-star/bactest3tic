...
(function () {
  ...
  const WATCHDOG_EVAL_TIMEOUT  = 20000;
  ...
  let speedStd    = 0;
  let signals     = [];
  let sessionTradesAll = [];
  let tickSeq                 = 0;
  let lastSignalTickIndex     = -999;
  let upStreak = 0;
  let downStreak = 0;
  let lastTickProcessedAt  = 0;
  let lastSignalEvalAt     = 0;
  let watchdogInterval     = null;
  let evalErrorCount       = 0;
  let lastSignalFiredAt    = 0;

  let realExecState   = 'IDLE';
  let realTrades      = [];
  let realOpenCount   = 0;
  let realWins        = 0;
  let realLosses      = 0;
  ...
  let lastTradeClosedTick = -999;
  let realExecTimer   = null;
  ...
  function buildOverlay () {
    ...
    el.innerHTML = `
      <div id="tt-header">
        ...
        <div class="tt-header-btns"><button id="tt-min-btn">_</button><button id="tt-close-btn">✕</button></div>
      </div>
      <div id="tt-body">
        ...
        <div class="tt-row"><span class="tt-label">S_Low/High</span><span class="tt-val" id="tt-speed-stats">0/0</span></div>
        <div class="tt-row"><span class="tt-label">Mean/Std</span><span class="tt-val" id="tt-speed-dist">0/0</span></div>
        ...
        <div class="tt-row"><span class="tt-label">State</span><span class="tt-val" id="tt-real-state">IDLE</span></div>
        <div class="tt-row"><span class="tt-label">PnL</span><span class="tt-val" id="tt-real-pnl">0.00</span></div>
        <button id="tt-real-export">⬇ Real CSV</button><button id="tt-real-reset">Reset Engine</button>
        <button id="tt-config-toggle">⚙ settings</button>
        <div id="tt-config">
          <div class="tt-config-row"><label>Mode</label><select id="tt-cfg-strategy-mode"><option value="momentum">Momentum</option><option value="reversal">Reversal</option><option value="structural">Structural</option><option value="hybrid">Hybrid</option></select></div>
          <div class="tt-config-row"><label>Epsilon</label><input type="number" id="tt-cfg-epsilon" step="0.01" value="0.2"></div>
          <div class="tt-config-row"><label>Debug</label><input type="checkbox" id="tt-cfg-debug"></div>
          <div class="tt-config-row"><label>Enable Real</label><input type="checkbox" id="tt-cfg-real-enabled"></div>
        </div>
  ...
  function makeDraggable (el) {
    ...
    function onUp () { document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); safeStorage('set', 'tt-pos', { left: parseInt(el.style.left), top: parseInt(el.style.top) }); }
  }
  ...
  function bindButtons (el) {
    ...
    document.getElementById('tt-real-reset').addEventListener('click', () => { if (confirm('Reset?')) { realExecState = 'IDLE'; realLockReason = ''; realOpenCount = 0; clearTimeout(realExecTimer); updateRealUI(); } });
  ...
  function connect () {
    ...
    ws.addEventListener('open', () => { setWsState('connected'); reconnectDelay = RECONNECT_BASE; failCount = 0; usingFallback = false; lastTickProcessedAt = Date.now(); lastSignalEvalAt = Date.now(); ws.send(JSON.stringify({ active_symbols: 'brief', product_type: 'basic' })); });
  ...
  function scheduleReconnect () { if (reconnectTimer) return; failCount++; if (failCount >= FALLBACK_AFTER) { usingFallback = !usingFallback; failCount = 0; } reconnectTimer = setTimeout(() => { reconnectTimer = null; connect(); }, reconnectDelay); reconnectDelay = Math.min(reconnectDelay * 2, RECONNECT_MAX); }
  function setWsState (state) { wsState = state; const el = document.getElementById('tt-status'); if (el) el.textContent = state.charAt(0).toUpperCase() + state.slice(1); }
  ...
  function updateSignalsUI () {
    ...
    el.innerHTML = ''; signals.slice(-10).reverse().forEach(sig => {
      ...
      div.innerHTML = `<span>${sig.type}</span><span>${(sig.entryPriceReal || sig.price).toFixed(2)}</span><span>${new Date(sig.time*1000).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'})} [${sig.confidence}%]</span>`;
      el.appendChild(div);
    });
  }
  ...
  function exportCSV () { if (!sessionTradesAll.length) return; const rows = [['Type','Strategy','Confidence','Price','Time','Result']].concat(sessionTradesAll.map(s => [s.type, s.strategy, s.confidence, s.price.toFixed(2), s.time, s.result])); const csv = rows.map(r => r.join(',')).join('\n'); const blob = new Blob([csv], {type:'text/csv'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = '3tick-signals.csv'; a.click(); }
  function exportRealCSV () { if (!realTrades.length) return; const rows = [['Time','Signal','Side','Result','PnL']].concat(realTrades.map(t => [new Date(t.time).toISOString(), t.signal, t.side, t.result, t.pnl || ''])); const csv = rows.map(r => r.join(',')).join('\n'); const blob = new Blob([csv], {type:'text/csv'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = '3tick-real.csv'; a.click(); }
  ...
  function updateRealUI () {
    const stEl = document.getElementById('tt-real-state'), winsEl = document.getElementById('tt-wins'), lossEl = document.getElementById('tt-losses'), pnlEl = document.getElementById('tt-real-pnl');
    ...
    if (winsEl) winsEl.textContent = realWins; if (lossEl) lossEl.textContent = realLosses; if (pnlEl) { pnlEl.textContent = realPnl.toFixed(2); pnlEl.style.color = realPnl >= 0 ? '#3ecf60' : '#e04040'; }
  }
  ...
  function setupFlyoutObserver () { if (flyoutObserver) return; flyoutObserver = new MutationObserver(() => { const flyout = document.querySelector(SEL_FLYOUT); if (!flyout) { if (realOpenCount !== 0) { realOpenCount = 0; updateRealExecStateFromDOM(0, null); } return; } const text = flyout.innerText; let count = text.includes('no open positions') ? 0 : (text.match(/(\d+)\s+open\s+position/i) ? parseInt(text.match(/(\d+)\s+open\s+position/i)[1], 10) : realOpenCount); let closedResult = null; const pnlMatch = text.match(/Closed\s+([+-]?\d+\.?\d*)\s+USD/i); if (pnlMatch) closedResult = { pnl: parseFloat(pnlMatch[1]), result: parseFloat(pnlMatch[1]) >= 0 ? 'WIN' : 'LOSS' }; else if (text.includes('no open positions') && realExecState === 'OPEN') closedResult = { pnl: 0, result: 'UNKNOWN' }; if (count !== realOpenCount || closedResult) { realOpenCount = count; updateRealExecStateFromDOM(count, closedResult); } }); flyoutObserver.observe(document.body, { childList: true, subtree: true, characterData: true }); }
  function updateRealExecStateFromDOM (count, closedResult) { if (closedResult && ['OPEN', 'OPEN_PENDING', 'RECOVERY'].includes(realExecState)) { finalizeRealTrade(closedResult); realExecState = 'IDLE'; realLockReason = ''; } else if (count === 0 && ['OPEN', 'RECOVERY'].includes(realExecState)) { finalizeRealTrade({ pnl: 0, result: 'UNKNOWN' }); realExecState = 'IDLE'; realLockReason = ''; } if (count > 0 && ['IDLE', 'OPEN_PENDING'].includes(realExecState)) { realExecState = 'OPEN'; const pending = signals.find(s => s.result === 'PENDING' && !s.entryPriceReal); if (pending && ticks.length) { pending.entryPriceReal = ticks[ticks.length - 1].price; updateSignalsUI(); } } updateRealUI(); }
  function finalizeRealTrade (res) { if (!realTrades.length) return; const last = realTrades[realTrades.length - 1]; if (last.result !== 'PENDING') return; last.result = res.result; last.pnl = res.pnl; if (res.result === 'WIN') realWins++; else if (res.result === 'LOSS') realLosses++; realPnl += res.pnl; const simTrade = signals.find(s => s.result === 'PENDING'); if (simTrade) { simTrade.result = res.result; simTrade.priceAfter = ticks.length ? ticks[ticks.length - 1].price : simTrade.price; } lastTradeClosedAt = Date.now(); lastTradeClosedTick = tickSeq; clearTimeout(realExecTimer); realExecTimer = null; updateRealUI(); }
  async function executeRealTrade (side) { if (Date.now() - lastRealTradeAt < cfg.realCooldownMs) return; const buyLabel = side === 'BUY' ? 'Rise' : 'Fall', activeClass = side === 'BUY' ? CLASS_RISE_ACTIVE : CLASS_FALL_ACTIVE; try { if (!await setRealTradeSide(buyLabel, activeClass)) throw new Error('side_failed'); if (!await waitRealBuyReady()) throw new Error('not_ready'); const btn = document.querySelector(SEL_PURCHASE_BTN); if (!btn || !btn.classList.contains(activeClass)) throw new Error('btn_mismatch'); simulateExternalClick(btn); lastRealTradeAt = Date.now(); realTrades.push({ time: Date.now(), signal: side, side: buyLabel, result: 'PENDING' }); realExecTimer = setTimeout(() => { if (['OPEN_PENDING', 'OPEN'].includes(realExecState)) { realExecState = 'RECOVERY'; realLockReason = 'TIMEOUT'; updateRealUI(); } }, cfg.realTimeoutMs); } catch (e) { realLockReason = 'ERR:' + e.message; updateRealUI(); setTimeout(() => { if (realExecState === 'OPEN_PENDING') { realExecState = 'IDLE'; realLockReason = ''; updateRealUI(); } }, 3000); } }
